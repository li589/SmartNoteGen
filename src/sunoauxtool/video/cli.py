"""sunoauxtool.video CLI：Typer 应用入口。
...
def main(
    ...
    if version:
        ...
        typer.echo(f"sunoauxtool.video {sunoauxtool.video.__version__}")
        raise typer.Exit()
...
def render(
    ...
    """
    # 导入主逻辑
    from sunoauxtool.video.videomaker import video, video_multitrack
...
def multi(
    ...
    from sunoauxtool.video.config import Config
    from sunoauxtool.video.presets import list_presets
...
def presets(
    ...
    """列出或查看平台预设。"""
    from sunoauxtool.video.presets import list_presets, get_preset
...
def config(
    ...
    if action == "init":
        target = path or "sunoauxtool.video.toml"
        cfg = Config()

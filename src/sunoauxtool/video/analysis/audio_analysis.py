...
- 复用 sunoauxtool.preview 的 compute_audio_features，保证与预览页口径一致
...
from sunoauxtool.preview import AudioFeatures, compute_audio_features
...
def analyze_multitrack(
    ...
    *,
    ...
    Args:
        mix: MixResult（sunoauxtool.video.mixer），含 mixed 立体声与 track_monos。
        fps: 视频帧率。
        n_bins: 频谱 bin 数。
        n_wave_points: 波形降采样点数。
...
def compute_audio_features_from_array(audio: np.ndarray, sr: int) -> AudioFeatures:
    ...
    与 sunoauxtool.preview.compute_audio_features 同口径。
    """

...
from sunoauxtool.video.compositor import Compositor
from sunoauxtool.video.config import Config
from sunoauxtool.video.output_manager import (
    VideoArtifactMeta,
    VideoOutputManager,
    VideoRunMeta,
)
from sunoauxtool.video.presets import resolve_preset
...
def video(
    ...
    *,
    ...
    from sunoauxtool.video.audio_io import load_audio
    ...
    if visual_style == "score":
        ...
        midi_src = score_midi
        if midi_src is None:
            from sunoauxtool.video.audio_io import detect_format
        ...
        beat_times = None
        if tempo_grid:
            from sunoauxtool.analysis.tempo import beat_grid, estimate_bpm
    ...
    try:
        ...
        version = sunoauxtool.video.__version__
    except Exception:
        version = ""
    ...
    output_mgr.write_metadata(
        VideoRunMeta(
            command=f"sunoauxtool.video render {audio_path} --preset {preset} --style {visual_style}",
            seed=seed,
...
def video_multitrack(
    ...
    *,
    ...
    # 4. 元数据
    try:
        import sunoauxtool.video as _vm
    ...
    output_mgr.write_metadata(
        VideoRunMeta(
            command=f"sunoauxtool.video multi-track ({len(specs)} tracks) --preset {preset} --style {visual_style}",
            seed=seed,
            started_at=datetime.now().isoformat(timespec="seconds"),
            duration_s=analysis.duration_s,
            version=version,

...
ScoreVisualizer：由 smartnotegen 的 score 排版数据（``Score`` 模型）驱动，
...
- 可选节拍网格（beat_times 来自 ``smartnotegen.analysis.tempo`` 测速）
  + BPM 标注
...
from videomaker.visuals.tracks import _parse_color
...
class ScoreVisualizer(Visualizer):
    ...
    Args:
        config: 生效配置。
        score: ``smartnotegen.score.Score``（None 时渲染提示帧）。
